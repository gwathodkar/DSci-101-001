...
question_2 <- CIACountries %>%
  summarise(Mean_population = mean(pop), 
            ...
            Standard_deviation = sd(pop))


# 3. What is the minimum value for the area and what is the maximum value 
...
question_3 <- CIACountries %>%
  summarise(Min_area = min(area), 
            Max_oil_production = max (oil_prod, na.rm = TRUE))
...
question_5 <- CIACountries %>%
  group_by(net_users) %>%
  summarise(Min_educ = min(educ, na.rm = TRUE), 
...
question_6 <- CIACountries %>%
  ...
  filter(net_users == ">35%" | net_users == ">5%")
...
question_8 <- CIACountries %>%
  mutate(Education_index = case_when(
    educ > 4.5 ~ "High", 
    educ <=4.5 ~ "Low", 
    TRUE ~ "No Information"
  )) %>%
  group_by(Education_index) %>%
...
question_9 <- CIACountries %>%
  ...
  group_by(Education_index, net_users) %>%
